...
**Model Choice (current):** Free-tier OpenRouter models with ordered fallback. Each AI tool passes `models: FREE_MODELS` from `_shared/ai.ts`: `thinkingmachines/inkling:free` (best, multilingual), then `nvidia/nemotron-3-ultra:free`, `dots-studio/dots3-note-preview:free`, `nvidia/nemotron-3-super:free` (worst). On HTTP 429/404/5xx the shared client (`_shared/ai.ts`) retries with backoff, then falls through to the next model.
...
* `ai-tools` (POST) - Resume builder + LinkedIn tuner via OpenRouter through the shared OpenAI-compatible helper (uses `FREE_MODELS`, starting with `thinkingmachines/inkling:free`, falling back through nvidia and dots-studio variants).
